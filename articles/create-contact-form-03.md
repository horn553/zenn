---
title: "【③ セッション管理】SvelteKit on Cloudflareでお問い合わせフォームをつくる"
emoji: "📯"
...
今回は SvelteKit x Cloudflare KV でセッション管理を実装します！
...
IPA のガイドを参考に、CSRF トークンを発行し、するように実装します。
...
こうすると 3 つの `id` / `preview_id` が生成されます。
...
[hooks](https://svelte.dev/docs/kit/hooks) とは、すべてのリクエストで呼び出される処理のことです。
...
`save()` を実行することを忘れないようにしないといけません！(1 敗)
...
```diff /src/routes/+page.ts
- export const prerender = true;
```

```diff /src/routes/about/+page.ts
- export const prerender = true;
```

```diff /src/routes/sverdle/how-to-play/+page.ts
- export const prerender = true;
```

```diff /src/routes/+layout.ts
...
送信すると、きちんと CSRF トークンの検証に成功し、`success: true` が返されたため、フォームが初期化されました。
