...
- Transformationする画像はリポジトリ上に用意する
...
エラー原因の特定に苦慮しましたが、enhanced-imgを利用する画像数を減らすことでエラーは出現しなくなりました。

最適化する画像を絞るにしても、これから増えていくコンテンツではいずれ同じ問題に直面するであろう……
Cloudflare PagesのCDは大変便利なので、何とか使い続けたい……
...
今回は、このうち[Transform via URL](https://developers.cloudflare.com/images/transform-images/transform-via-url/)を利用します。

### 利用料金

5,000 unique transformations/月まで無料で利用できます。
同じ形式へのtransformationは月に1度しかカウントされないようです。
...
例えば、当団が利用している`.tokyo`ドメインはCloudflareへ移管できません。
...
Cloudflare DNSのセットアップが完了したら、CloudflareダッシュボードでCloudflare ImagesのTransformationsを有効化する必要があります。
...
最近の流行りのフォーマットはWebPやAVIFでしょうか。

AVIFはより高い圧縮率を誇ります。しかし、未対応ブラウザがあり……Can I Useで調べると……
と考えるのは楽しいですが、Cloudflare Imagesの前にはそのような検討は不要です。
...
また、小さい画像に対しわざわざ拡大されないよう、`fix=scale-down`を指定しておきます。
...
この制限は、ダッシュボード上で簡単に無効化できますが、今回はこのデフォルト設定を尊重することとしました。
...
- オプションはformat、fit、height
  ...
  - `height=3840`は高ピクセル密度ディスプレイ向けに用意する
    - imgタグのsrcset属性を使用して設定する
...
仕様の把握と設計に時間がかかりましたが、全容を把握できれば簡潔に実装できました。
ビルドエラーも消失し、ビルド時間も1分足らずまで短縮されています。

ダッシュボードからは、削減された帯域幅を確認できます。

![ダッシュボードから確認できる帯域幅](/images/sveltekit-cloudflare-images/01.png)

きれいなダッシュボードが用意された、便利なサービスを無料で利用させていただき、ありがたいものです。
これからも、Cloudflareを活用する日々は続きそうです。
